...
void bsr_vmspmv3(bsr_matrix *A, const double *x, double *y)
{
    ...
    for(int i=0;i<nrows;i++)
    {
        ...
        for(int k=rowptr[i];k<rowptr[i+1];k++)
        {
            ...
            vA[0] += _mm256_cvtps_pd(_mm_loadu_ps(AA+0))*_mm256_permute4x64_pd(vx,0x00); //0x00
            vA[1] += _mm256_cvtps_pd(_mm_loadu_ps(AA+3))*_mm256_permute4x64_pd(vx,0x55); //0x55
            vA[2] += _mm256_cvtps_pd(_mm_loadu_ps(AA+6))*_mm256_permute4x64_pd(vx,0xAA); //0xAA
        }
...
void bsr_vdspmv3(bsr_matrix *A, const double *x, double *y)
{
    ...
    for(int i=0;i<nrows;i++)
    {
        ...
        for(int k=rowptr[i];k<rowptr[i+1];k++)
        {
            ...
            vA[0] += _mm256_loadu_pd(AA+0)*_mm256_permute4x64_pd(vx,0x00); //0x00
            vA[1] += _mm256_loadu_pd(AA+3)*_mm256_permute4x64_pd(vx,0x55); //0x55
            vA[2] += _mm256_loadu_pd(AA+6)*_mm256_permute4x64_pd(vx,0xAA); //0xAA
        }
