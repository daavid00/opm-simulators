...
void mat3_rmul(double *A, double const *B)
{
    ...
    for(int j=0;j<3;j++)
    {
        ...
        __m256d vAB[3];
        vAB[0] = vA[0]*_mm256_permute4x64_pd(vbj,0x00); //0x00
        vAB[1] = vA[1]*_mm256_permute4x64_pd(vbj,0x55); //0x55
        vAB[2] = vA[2]*_mm256_permute4x64_pd(vbj,0xAA); //0xAA
...
void prec_mapply3c(prec_t *restrict P, double *x)
{
    ...
    for(int i=0;i<L->ncols;i++)
    {
        ...
        vx[0] = _mm256_permute4x64_pd(vxi,0x00); //0x00
        vx[1] = _mm256_permute4x64_pd(vxi,0x55); //0x55
        vx[2] = _mm256_permute4x64_pd(vxi,0xAA); //0xAA
        for(int k=L->rowptr[i];k<L->rowptr[i+1];k++)
        {
    ...
    for(int i=U->ncols;i>0;i--)
    {
        ...
        for(int k=U->rowptr[i]-1;k>U->rowptr[i-1]-1;k--)
        {
            ...
            vA[0] += _mm256_cvtps_pd(_mm_loadu_ps(A+0))*_mm256_permute4x64_pd(vxj,0x00); //0x00
            vA[1] += _mm256_cvtps_pd(_mm_loadu_ps(A+3))*_mm256_permute4x64_pd(vxj,0x55); //0x55
            vA[2] += _mm256_cvtps_pd(_mm_loadu_ps(A+6))*_mm256_permute4x64_pd(vxj,0xAA); //0xAA
        }
        double *xi = x+b*(i-1);
        __m256d vxi = _mm256_loadu_pd(xi);
...
void prec_dapply3c(prec_t *restrict P, double *x)
{
    ...
    for(int i=U->ncols;i>0;i--)
    {
        ...
        for(int k=U->rowptr[i]-1;k>U->rowptr[i-1]-1;k--)
        {
            ...
            int j=U->colidx[k];
            __m256d vxj = _mm256_loadu_pd(x+b*j);
            vA[0] += _mm256_loadu_pd(A+0)*_mm256_permute4x64_pd(vxj,0x00); //0x00
            vA[1] += _mm256_loadu_pd(A+3)*_mm256_permute4x64_pd(vxj,0x55); //0x55
            vA[2] += _mm256_loadu_pd(A+6)*_mm256_permute4x64_pd(vxj,0xAA); //0xAA
        }
        double *xi = x+b*(i-1);
        __m256d vxi = _mm256_loadu_pd(xi);
        __m256d vz = (vxi - vA[0]) - (vA[1] + vA[2]);
