...
int Opm::Main::dispatchDynamic_()
{
    ...
    // Biofilm module requires two-phase water-gas
    else if (rspec.biof()) {
...
int Opm::Main::runBiofilm(const Phases& phases)
    {
        if (!(phases.active(Phase::WATER) && phases.active(Phase::GAS)) || (phases.size() != 3)) {
            if (outputCout_) {
                std::cerr << "Biofilm option can only be used for thermal two-phase water/gas "
                          << "model (i.e. in combination with WATER, GAS, and THERMAL)." << std::endl;
            }
