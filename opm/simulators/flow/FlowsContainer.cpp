               ...
               std::function<bool(const int)> isInterior)
{
    ...
    if (summaryConfig.keywords("BFLO*").size() > 0 && !anyFlows_) {
        ...
        blockFlowsAllIds_.resize(summaryConfig.keywords("BFLO*").size()+1, -1);
        std::size_t ind = 0;
        for (unsigned ii = 0; ii < phaseIdxs.size(); ++ii) {
            if (FluidSystem::phaseIsActive(phaseIdxs[ii])) {
                for (unsigned jj = 0; jj < dirIdxs.size(); ++jj) {
                    ...
                    blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]].resize(summaryConfig.keywords(blockid).size(), -1);
                    std::size_t index = 0;
                    for (const auto& node : summaryConfig.keywords(blockid)) {
                        if (isInterior(node.number() - 1)) {
                            blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]][index++] = node.number() - 1;
                            blockFlowsAllIds_[ind++] = node.number() - 1;
                        }
                    }
                    std::sort(blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]].begin(), blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]].end());
                    auto last = std::unique(blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]].begin(), blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]].end());
                    blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]].erase(last, blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]].end());
                    if (blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]][0] == -1) {
                        blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]].erase(blockFlowsIds_[compIdxs[ii]][dirIdxs[jj]].begin());
                    }
                }
            }
        }
        ...
        if (blockFlowsAllIds_[0] == -1) {
            blockFlowsAllIds_.erase(blockFlowsAllIds_.begin());
        }
    }
}
