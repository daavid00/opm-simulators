...
private:
    ...
    void createFlows_()
    {
        ...
        for (const auto& elem : elements(gridView_())) {
            ...
            for (unsigned primaryDofIdx = 0; primaryDofIdx < stencil.numPrimaryDof(); ++primaryDofIdx) {
                const unsigned myIdx = stencil.globalSpaceIndex(primaryDofIdx);
                const int global = simulator_().vanguard().cartesianIndex(myIdx);
                bool blockFlowFound = false;
                if (!blockFlows.empty()) {
                    if (std::binary_search(blockFlows.begin(), blockFlows.end(), global)) {
                        blockFlowFound = true;
                    }
                    else {
        ...
        for (unsigned globI = 0; globI < numCells; ++globI) {
            ...
            // Flux term.
            {
                ...
                for (const auto& nbInfo : nbInfos) {
                    ...
                    if (!blockFlows.empty()) {
                        const int global = simulator_().vanguard().cartesianIndex(globI);
                        if (std::binary_search(blockFlows.begin(), blockFlows.end(), global)) {
                            for (unsigned eqIdx = 0; eqIdx < numEq; ++eqIdx) {
