...
public:
    ...
    {
        if constexpr (enableBioeffects)
            model.addOutputModule(new VtkBlackOilBioeffectsModule<TypeTag>(simulator));
    }
        ...
        if constexpr (enableBioeffects) {
            if constexpr (enableMICP) {
                ...
                flux[contiBiofilmEqIdx] +=
                    decay<UpEval>(upFs.biofilmConcentration())
                    * volumeFlux
                    * decay<UpEval>(upFs.biofmobt());
            }
        }
    }
                          ...
                          unsigned globalSpaceIdex)
    {
        if constexpr (enableBioeffects) {
            ...
            if constexpr (enableMICP) {
                ...
                Evaluation k_g = mu * (xG * rhow / (xG * rhow + k_n));
                if (xG < 0)
                    k_g = mu * (xG * rhow / k_n);

                // compute source terms
                // biofilm growth and decay rate
                source[contiBiofilmEqIdx] += (k_g - k_d) * cBiof;
    ...
    static const TabulatedFunction& biofmobtTable(unsigned satnumRegionIdx)
    {
        return params_.biofmobtTable_[satnumRegionIdx];
    }
    ...
    static bool hasBiofmobtTables()
    {
        if constexpr (enableBioeffects && !enableMICP) {
            return !params_.biofmobtTable_.empty();
        }
        else {
            return false;
        }
...
public:
                                     ...
                                     unsigned timeIdx)
    {
        ...
        biofmobt_ = 0.0;
        const auto& biofmobtTable = BioeffectsModule::biofmobtTable(satnumRegionIdx);
        if (BioeffectsModule::hasBiofmobtTables()) {
            const Evaluation temperature = priVars.makeEvaluation(temperatureIdx, timeIdx, linearizationType);
            biofmobt_ = biofmobtTable.eval(temperature, /*extrapolation=*/true);
        }
    }
    ...
    const Evaluation& biofmobt() const
    { return biofmobt_; }
