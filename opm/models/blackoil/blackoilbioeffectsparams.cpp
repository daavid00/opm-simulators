...
initFromState(const EclipseState& eclState)
{
    ...
    if constexpr (enableMICP) {
        ...
        const TableContainer& biofmobtTables = tableManager.getBiofmobtTables();
        if (!biofmobtTables.empty()) {
            biofmobtTable_.resize(numSatRegions);
            for (size_t i = 0; i < biofmobtTables.size(); ++i) {
                const BiofmobtTable& biofmobtTable = biofmobtTables.getTable<BiofmobtTable>(i);
                biofmobtTable_[i].setXYContainers(biofmobtTable.getTemperatureColumn(), biofmobtTable.getBiofmobtColumn());
            }
        }
    }
